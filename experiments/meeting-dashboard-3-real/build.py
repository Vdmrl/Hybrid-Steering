...
<p class="muted">Это исправленная версия Dashboard 3: она читает именно завершённый summary эксперимента 3, а не старый factorial-run. Здесь __CONDITION_COUNT__ условий, __TEST_N__ тестовых промптов и Judge с вероятностным expected score. Построено __GENERATED__.</p>
...
</div><h3>Feature-level delta: RSS rank 4 − baseline</h3><table><tr><th>Признак</th><th>Baseline</th><th>RSS rank 4</th><th>Δ, баллы</th></tr>${FEATURES.map(f=>`<tr><th>${F(f)}</th><td>${fmt(base.feature_expected_means[f])}</td><td>${fmt(full.feature_expected_means[f])}</td><td class="${cls(full.feature_expected_means[f]-base.feature_expected_means[f])}">${scoreDelta(full.feature_expected_means[f]-base.feature_expected_means[f])}</td></tr>`).join('')}</table><div class="viz-grid"><article class="viz-card"><h3>Профиль признаков, шкала 1–5</h3>${profileChart}<p class="small">Полоса показывает expected score; это не доля ответов.</p></article><article class="viz-card"><h3>Forest plot paired-контрастов</h3><p class="small">Ноль — отсутствие разницы; диапазон графика −0.2…+0.2 балла.</p>${forestChart}</article></div><p class="source">Если Exp4 не завершится к показу, этот Dashboard 3 уже содержит самостоятельный завершённый результат: baseline, GDN/activation, rank, RSS, пары, тройки, all-four и quality.</p>`;
